...
public class spawn : MonoBehaviour {
	...
	void Update ()
	{
				...
				if (chosen.GetComponent<Rigidbody> () != null) {
					...
					if (OVRInput.GetDown (OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.Touch) == true) {
						...
						//If copy target is not part of group
							...
							copyObj = Instantiate (chosen, new Vector3 (transform.position.x, transform.position.y + 0.4f, transform.position.z + 0.5f), transform.rotation) as GameObject;
				...
				if (OVRInput.GetDown (OVRInput.Button.Three, OVRInput.Controller.Touch) == true) {
					...
					if (chosen.transform.root != chosen.transform && chosen.GetComponent<Rigidbody> () != null) {
						...
						if (Group.Count != 0) {
							...
							//GEmpty.transform.rotation = Group [0].transform.rotation;
						...
						if (GEmpty.transform.childCount == 0) {
							...
							//GEmpty.transform.rotation = chosen.transform.rotation;
		...
		if (spwnMode == true) {
			...
			if (OVRInput.GetDown (OVRInput.Button.PrimaryHandTrigger, OVRInput.Controller.Touch) == true) {
				//Destroy (miniObj);
				fullObj = Instantiate (deckObjs [whichDeckObj], new Vector3 (transform.position.x, transform.position.y + 0.4f, transform.position.z + 0.5f), transform.rotation) as GameObject;
